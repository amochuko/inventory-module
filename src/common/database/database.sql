...
CREATE TABLE inventory.categories (
    ...
    abbre_code VARCHAR(20) NOT NULL UNIQUE,
    description TEXT,
...
CREATE TABLE inventory.products (
    ...
    description TEXT,
    category_id INTEGER REFERENCES inventory.categories(id) ON DELETE
    SET NULL,
        supplier_id INTEGER REFERENCES inventory.suppliers(id) ON DELETE
    SET NULL,
...
CREATE TABLE inventory.transactions (
    ...
    -- OUT transactions
    profit DECIMAL(10, 2),
    -- Optionally calculated on OUT
    occurred_at TIMESTAMPTZ DEFAULT NOW(),
    reference TEXT,
    -- e.g., invoice no., sale ID, etc
    notes TEXT,
    -- optional remarks
...
CREATE OR REPLACE FUNCTION inventory.update_stock_level() RETURNS TRIGGER AS $$
DECLARE new_quantity INTEGER;
BEGIN IF NEW.transaction_type = 'IN' THEN
...
AFTER
INSERT ON inventory.transactions FOR EACH ROW EXECUTE FUNCTION inventory.update_stock_level();
